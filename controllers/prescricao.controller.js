...
const create = async (req, res) => {
  try {
    ...
    const prescricao = await PrescricaoService.create(req.body);
...
const listar = async (req, res) => {
    try {      
        ...
        const prescricoes = await PrescricaoService.listarPaginado(page, limit);
        ...
        // Resposta para o cliente
        res.status(200).send(prescricoes);
    } catch (error) {
...
const buscarPorId = async (req, res) => {
  try {
    ...
    if (!id) {
      return res.status(400).json({ erro: "ID da prescrição é obrigatório!" });
    }

    const consulta = await PrescricaoService.buscarPorId(id);
...
const excluir = async (req, res) => {
  try {
    ...
    const resultado = await PrescricaoService.buscarPorId(id);
    ...
    const prescricao = await PrescricaoService.excluir(id);
...
const editar = async (req, res) => {
    try {
        //Receber os dados de um formulário e desmembrar os dados
        const { data, consultaId, medicamento, dosagem, instrucoes, file } = req.body;
        ...
        await PrescricaoService.editar(id, data, consultaId, medicamento, dosagem, instrucoes, file);
